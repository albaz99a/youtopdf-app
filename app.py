...
# 2. إخفاء القوائم والعناصر المزعجة وتنسيق الواجهة
st.markdown("""
<style>
    /* إخفاء القائمة الجانبية والهيدر الافتراضي لستريمليت */
...
with col_lang:
    # اختيار اللغة يظهر دائماً كأزرار بسيطة في أعلى اليمين
...
if lang == "العربية":
    ...
    t_priv = "🔒 الخصوصية: لا يتم تخزين ملفاتك، المعالجة فورية وتتم في الذاكرة المؤقتة."
    ...
    t_priv = "🔒 Privacy: No files are stored, processing is instant in-memory."
...
# إدارة الحالة لتجنب القوائم المنبثقة
...
for i in range(5):
    with cols[i]:
        ...
        if st.button(labels[i], key=f"nav_btn_{i}"):
            st.session_state.active = labels[i]

st.divider()

# 6. منطقة العمل (تم إصلاح المنطق البرمجي هنا لتجنب خطأ الأقواس)
